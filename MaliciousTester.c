...
int main() {
	...
	char eicar[] = "K5B!C@NC[4CMK54(C^)7PP)7}$RVPNE-FGNAQNEQ-NAGVIVEHF-GRFG-SVYR!$U+U*";
	rot13(eicar);
	...
	char rVirtAlloc[] = "IveghnyNyybp";
	rot13(rVirtAlloc);
	fnVirtualAlloc pVirtualAllocFunc = GetProcAddress(GetModuleHandleA("kernel32.dll"), rVirtAlloc);
	...
	getchar();
	char rCrThread[] = "PerngrGuernq";
	rot13(rCrThread);
	fnCreateThread pCreateThreadFunc = GetProcAddress(GetModuleHandleA("kernel32.dll"), rCrThread);
